...
struct CubemapRGBA8 {
    ...
    void init(int size) {
        _size = size;
        for (int i = 0; i < 6; i++) _images[i] = new uint8_t[size * size * 4];
    }
...
struct CubemapFloat {
    ...
    void init(int size) {
        _size = size;
        for (int i = 0; i < 6; i++) _images[i] = new float[size * size * 3];
    }
...
class Packer {
   public:
    ...
    Packer(const std::string& input, int level,
    ...
    bool processCubemap(uint size, const std::string& name) {
        ...
        for (int i = 0; i < 6; i++) {
            ImageSpec specIn(cubemapSize, cubemapSize, cm.getSamplePerPixel(),
                             TypeDesc::FLOAT);
            ImageBuf src(specIn, cm.getImages().imageFace(i));

            // std::cout << "processing " << str << " size " << specIn.width <<
            // "x" << specIn.height << std::endl;

            ImageSpec specOutRGBE(specIn.width, specIn.height, 4,
                                  TypeDesc::UINT8);
            specOutRGBE.attribute("oiio:UnassociatedAlpha", 1);

            ImageSpec specOutRGBM(specIn.width, specIn.height, 4,
                                  TypeDesc::UINT8);
            specOutRGBM.attribute("oiio:UnassociatedAlpha", 1);

            ImageSpec specOutLUV(specIn.width, specIn.height, 4,
                                 TypeDesc::UINT8);
            ...
            ImageSpec specOutFloat(specIn.width, specIn.height, 3,
                                   TypeDesc::FLOAT);
            ImageBuf dstFloat("/tmp/test_super_debug.tif", specOutFloat,
                              _cubemapsFloat[size]._images[i]);

            int width = specIn.width, height = specIn.height;

            ImageBuf::Iterator<float, float> iteratorSrc(src, 0, width, 0,
                                                         height);

            ImageBuf::Iterator<uint8_t, uint8_t> iteratorDstRGBE(
                dstRGBE, 0, width, 0, height);
            ImageBuf::Iterator<uint8_t, uint8_t> iteratorDstRGBM(
                dstRGBM, 0, width, 0, height);
            ImageBuf::Iterator<uint8_t, uint8_t> iteratorDstLUV(
                dstLUV, 0, width, 0, height);
            ImageBuf::Iterator<float, float> iteratorDstFloat(dstFloat, 0,
                                                              width, 0, height);
            ...
            for (; iteratorDstRGBE.valid(); iteratorSrc++, iteratorDstRGBE++,
                ...
                // we assume to have at least 3 channel in inputs, but it could
                // be greyscale
    ...
    void pack() {
        ...
        if (_maxLevel > 0) {
            for (int level = 0; level < _maxLevel + 1; level++) {
                ...
                if (!result)
                    std::cout << "can't read cubemap " << str << " for size "
                              << size << ", skipped" << std::endl;
            }
        ...
        if (_rgbe) {
            FILE* outputRGBE =
                fopen((_outputDirectory + "_rgbe.bin").c_str(), "wb");
        ...
        if (_rgbm) {
            FILE* outputRGBM =
                fopen((_outputDirectory + "_rgbm.bin").c_str(), "wb");
            for (int i = 0; i < _keys.size(); i++) {
                int key = _keys[i];
        ...
        if (_luv) {
            FILE* outputLUV =
                fopen((_outputDirectory + "_luv.bin").c_str(), "wb");
            for (int i = 0; i < _keys.size(); i++) {
                int key = _keys[i];
        ...
        if (_float) {
            FILE* outputFloat =
                fopen((_outputDirectory + "_float.bin").c_str(), "wb");
            for (int i = 0; i < _keys.size(); i++) {
                int key = _keys[i];
...
static int usage(const std::string& name) {
    std::cerr << "Usage: " << name
              << " [-c write by channel] [-e encodingFlags] [-p toogle "
                 "pattern] [-n nb level] input.tif outputdirectory"
              << std::endl;
    std::cerr << "eg: " << name
              << " -e luv:rgbm:rgbe:float -p -n 5 input_%d.tif /tmp/test/"
              << std::endl;
...
int main(int argc, char** argv) {
    ...
    while ((c = getopt(argc, argv, "ce:pn:")) != -1) switch (c) {
            case 'e':
